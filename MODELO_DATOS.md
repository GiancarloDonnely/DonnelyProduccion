...
`NP PEDIDO` · `CÓD.` / `CÓDIGO` · `FECHA` · `TIPO` (entrada/salida) · `CANTIDAD` · `BODEGA` · `GUÍA` · `DESCRIPCIÓN` · `ESTADO`

> Esta hoja alimenta la etapa **📦 Empaque** de la Trazabilidad, cruzada por **NP + Código**. Registra la entrada del producto a bodega y su salida al despacharse (con número de guía). Para conectarla en vivo, agrega su URL CSV en `data/urls_planillas.json` bajo la clave `"Inventario"`.
...
El **medidor (gauge)** de la pestaña Capacidad usa como referencia la **capacidad instalada estimada**, que se calcula como el mejor mes histórico de throughput total (corte + confección + bordado + despacho). El estado (ociosa / normal / alta carga / sobrecarga) se determina comparando la carga del último mes con datos contra ese máximo.

## Detección del Buzo Desechable

El buzo desechable se identifica **automáticamente** por el nombre del producto: cualquier fila cuyo producto, categoría o código contenga las palabras `BUZO`, `TYVEK`, `PROMAN` o `DESECHABLE` se clasifica como buzo desechable y alimenta la pestaña 🦺 dedicada.

**Alcance por cliente:** el análisis de buzos considera únicamente al cliente **COMPAÑIA MINERA MANTOS**. Cualquier buzo desechable de otro cliente no se incluye en esta pestaña. Este filtro está definido en la constante `BUZO_CLIENTE` dentro de `index.html`; si en el futuro quieres incluir más clientes, se edita ahí.

Para que la detección funcione con datos en vivo, la planilla P1 debe tener una columna con el nombre o descripción del producto (`PRODUCTO`, `DESCRIPCIÓN` o `DETALLE`). El dashboard ya la lee. No requiere ninguna columna nueva ni marcar los buzos manualmente: basta con que el nombre del producto los identifique como hasta ahora.
